...
The native target's `apps/ios/NME/Resources/Info.plist` therefore declares
`ITSAppUsesNonExemptEncryption: true`. Do not flip that to `false` to skip the
questionnaire — it is a declaration to a government agency, not a checkbox.
...
Meeting links only open the app once the domain vouches for it. The Apple file
is configured for `WC955H63L3.com.ctrlaltl.nme`; Android's file still needs the
Play App Signing fingerprint. See [app-links.md](./app-links.md) for the exact
values and the two mistakes that cost people days.

The repository file is ready, but the live endpoint was still serving the
placeholder Apple identifier when checked on 3 August 2026. Deploy the web build
and repeat the checks below before device testing.
...
```bash
# iOS — native SwiftUI, generated by XcodeGen, no CocoaPods
...
The iOS deployment URLs are release build settings in
`apps/ios/Configuration/Shared.xcconfig`. Android can point at another
deployment with `NME_HOST`:

```bash
NME_HOST=meet.example.com npm run prebuild -w @nme/mobile
```

For an App Store build, open the generated native project and archive from
Xcode (Product → Archive), or use the archive command above. Run the archive on
a physical-device-capable Mac with the `WC955H63L3` team configured. Before
upload, complete an encrypted browser ↔ iPhone call and an iPhone ↔ iPhone call;
the simulator cannot validate camera capture, audio routing, or native frame
encryption interoperability. Generate Android's signed AAB from Android Studio.
...
- [x] Repository `apple-app-site-association` filled in with the real Team ID
- [ ] Updated Apple association file deployed; the live endpoint still served
      the placeholder identifier on 3 August 2026 (its content type is already
      `application/json`)
